...
data_input  = 'tau1 vs tau2\\1_1200_t50_t500_b099_bg001.txt'
data_output = 'tau1 vs tau2\\out_1_1200_t50_t500_b099_bg001.txt'
...
def l_curve(U, sm, b):
    ...
    if export_lcurve:
        global data_output
        # hardcoded for .txt
        filename = data_output[:-4] + '_lcurve' + data_output[-4:]

        # save first the optimal parameter, and then the rest.
        np.savetxt(filename, np.r_[[[rho_c, eta_c]],
                                   [[float(x), float(y)] for x, y in zip(rho, eta)]],
                   header='first line contains rho_c, eta_c')

    return  reg_corner, rho, eta, reg_param

def l_corner(rho, eta, reg_param, U=None, s=None, b=None, order=10):
    """
    ...
    Args:
        ...
        eta (numpy.ndarray): Vector of solution norms.
        reg_param (numpy.ndarray): Vector of regularization parameters.
        U (numpy.ndarray, optional): m x n matrix containing left singular vectors of A.
        s (numpy.ndarray, optional): Column vector containing singular values of A.
        b (numpy.ndarray, optional): Column vector containing the right-hand side of A*x=b.
...
def main(data_input, data_output, plot=True):
    ...
    print('Regularization parameter:  %.6f' % reg_corner[0])
